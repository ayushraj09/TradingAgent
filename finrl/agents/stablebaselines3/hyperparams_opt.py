...
def sample_ppo_params(trial: optuna.Trial) -> dict[str, Any]:
    ...
    gamma = trial.suggest_categorical(
        "gamma", [0.98, 0.99, 0.995, 0.999, 0.9999]
    )
...
def sample_td3_params(trial: optuna.Trial) -> dict[str, Any]:
    ...
    learning_rate = trial.suggest_loguniform("learning_rate", 1e-5, 1)
    batch_size = trial.suggest_categorical(
        "batch_size", [64, 100, 128, 256, 512, 1024, 2048]
    )
    buffer_size = trial.suggest_categorical(
        "buffer_size", [int(1e4), int(1e5), int(1e6)]
    )
    ...
    if noise_type == "normal":
        hyperparams["action_noise"] = "normal"
        hyperparams["noise_std"] = noise_std
    elif noise_type == "ornstein-uhlenbeck":
        hyperparams["action_noise"] = "ornstein_uhlenbeck"
        hyperparams["noise_std"] = noise_std
