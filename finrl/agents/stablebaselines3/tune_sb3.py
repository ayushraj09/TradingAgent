...
class TuneSB3Optuna:
    ...
    def objective(self, trial: optuna.Trial):
        # Some hyperparameter samplers expect environment-dependent attributes
        # (for example, number of actions or HER settings) to be present on
        # the trial object. Ensure those are set from the training env so
        # samplers in hyperparams_opt.py can access them without failing.
        # Attempt to read action space size from the vectorized env first,
        # falling back to inspecting the underlying envs when needed.
        try:
            n_actions = None
            # Vec envs provide action_space on the wrapper
            if hasattr(self.env_train, "action_space") and getattr(self.env_train.action_space, "shape", None) is not None:
                ...
                # Try underlying envs (DummyVecEnv / SubprocVecEnv)
                if hasattr(self.env_train, "envs") and len(self.env_train.envs) > 0:
                    n_actions = int(self.env_train.envs[0].action_space.shape[-1])
        except Exception:
            n_actions = None

        # Attach attributes directly on the trial object so existing samplers
        # that access trial.n_actions or trial.using_her_replay_buffer continue
        # to work. Default to conservative values when unknown.
        try:
            setattr(trial, "n_actions", n_actions)
        except Exception:
            # If we cannot set attribute, ignore and samplers will fall back
            pass

        # By default assume HER is not used. If you use HER, set these
        # attributes externally (for example via study.user_attrs) before
        # running the tuner.
        try:
