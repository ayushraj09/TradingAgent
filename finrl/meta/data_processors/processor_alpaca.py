...
class AlpacaProcessor:
    ...
    def fetch_latest_data(
        ...
        for tic in ticker_list:
            ...
            original_cols = list(barset.columns)
            
            # Rename the existing 'symbol' column if it exists
            if "symbol" in barset.columns:
                barset.rename(columns={"symbol": "symbol_old"}, inplace=True)

            # Handle MultiIndex if present - check before reset_index
            if isinstance(barset.index, pd.MultiIndex):
                barset = barset.reset_index()
            elif barset.index.name is not None:
                # Index has a name, reset it to make it a column
                barset = barset.reset_index()
            ...
            # Check if timestamp exists in columns or index
            if "timestamp" in barset.columns:
                barset.set_index("timestamp", inplace=True)
            elif hasattr(barset.index, 'name') and barset.index.name == "timestamp":
                # Already has timestamp as index
                pass
            elif "time" in barset.columns:
                barset.rename(columns={"time": "timestamp"}, inplace=True)
                barset.set_index("timestamp", inplace=True)
            
            # Normalize column names to lowercase for easier matching
            barset.columns = [str(col).lower() if isinstance(col, str) else col for col in barset.columns]
            
            # Reorder and rename columns as needed - only select columns that exist
            required_cols_lower = ["close", "high", "low", "trade_count", "open", "volume", "vwap", "symbol"]
            available_cols = [col for col in barset.columns if col in required_cols_lower]
            
            # If we have at least some required columns, use them
            if available_cols:
                barset = barset[available_cols]
            # If DataFrame is not empty but no required columns, keep what we have
            elif not barset.empty:
                # Keep all columns but log what we got
                pass
